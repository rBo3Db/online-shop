...
Router.prototype.dispatch = function(history) {
    ...
    if (history.location.pathname === '/goods') {
        ...
        this.currentPreseners = [new HeaderPresenter(history),  new ItemsMainPresenter(history), new PopupPresenter(history), new LeftSideBarPresenter(history), new goodsInGridPresenter(history)];
        return this.currentPreseners;
    }
    // if (history.location.pathname === '/goods') {
    //     this.clean();
